/*********************************************************************
 *  This program was prepared for digital image processing class, the 
 *  program was from the opensource with the credit to: 
 *                                           
 *  readBMP.c
 *
 ...
 *  Free to good home!
 *
 * Note(Harry 2013-8-25): 
 *       1. use this program together with bmp.c program to compile and 
 *          build to form executable. 
 *       2. compile and build instruction: 
 *         gcc bmp.c readBMP.c -o bmp.o -lGL -lGLU -lglut  
 *    be sure to have both program bmp.c and readBMP.c and   
...
/* Simple BMP reading code, should be adaptable to many systems. Originally from 
   Windows, ported to Linux, now works on my Mac OS system.   
...
// Code and comments for adaptation to big endian/little endian system 

// Nina Amenta '04
//
/* Reads a long 32 bit integer; comment out one or the 
other shifting line below, 
...
/* LITTLE VS BIG ENDIAN LINES - comment out 
one or the other */
i = (b[3]<<24) | (b[2]<<16) | (b[1]<<8) | b[0]; 
// big endian
//i = (b[0]<<24) | (b[1]<<16) | (b[2]<<8) | 

b[3]; // little endian
return i;
}

/* Reads a 16 bit integer; comment out one or the other 
shifting line below, 
   whichever makes your system work right. */
